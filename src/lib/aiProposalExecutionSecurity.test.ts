...
const costGuardMigration=readFileSync(new URL('../../supabase/migrations/202608210010_ai_generation_cost_guards.sql',import.meta.url),'utf8');
...
describe('RC executor expansion and grants',()=>{
  ...
  it('rate limits through an atomic service-only RPC',()=>{expect(rcMigration).toContain('create table public.ai_rate_limit_windows');expect(costGuardMigration).toContain('if v_count > 5');expect(costGuardMigration).toContain('if v_count > 20');expect(costGuardMigration).toContain('if v_count > 25');expect(costGuardMigration).toContain('if v_count > 100');expect(costGuardMigration.match(/on conflict/g)).toHaveLength(4);expect(costGuardMigration).toContain('grant execute on function public.claim_project_ai_request(uuid, uuid) to service_role')});
});
